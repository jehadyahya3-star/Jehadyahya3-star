...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const handleExportDashboardToGoogleDoc = async () => {
    try {
      ...
      docText += ` - إجمالي ساعات عمل المعدات: ${totalWorkHours} ساعة\n`;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2 text-amber-400 font-bold text-xs uppercase tracking-wider mb-1">
              ...
              <span>{projectInfo.location || 'الرياض - المملكة العربية السعودية'}</span>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-5">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="text-2xl font-extrabold text-slate-900 tracking-tight">
            {totalWorkHours.toLocaleString('ar-SA')} <span className="text-xs font-semibold text-slate-500">ساعة</span>
          </div>
        ...
        <div className={`p-5 rounded-2xl border shadow-sm transition-shadow ${
          currentDieselBalance < 1000 
            ? 'bg-rose-50/70 border-rose-200 text-rose-950' 
            : 'bg-white border-slate-200'
        }`}>
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500">رصيد مخزن الديزل</span>
            <div className={`p-2.5 rounded-xl ${
              currentDieselBalance < 1000 ? 'bg-rose-500 text-white' : 'bg-amber-50 text-amber-600'
            }`}>
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-slate-500 pt-2 border-t border-slate-100">
            <span>وارد: {totalDieselReceived} لتر</span>
            <span>صادر: {totalDieselConsumed} لتر</span>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {reports.slice(0, 5).map((report) => (
                <tr key={report.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3.5 text-center font-bold text-blue-600">{report.totalNetHours} س</td>
